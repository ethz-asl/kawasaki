...
def establish_socket_connection(socket_connection, address_port,
    ...
    message = socket_connection.recvfrom(const.RECEIVE_MSG_SIZE)
    if (message == ps.login_string_tuple_creator(socket_number)):
...
def get_state(connection_socket):
    ...
    if (message[0][0:66] == const.STATE_STRING_PARTIAL
        ...
        joint_state_message, pose_state_message = ps.state_message_parser(
...
def set_state(pose_msg, connection_socket):
    ...
    """
    command_string = ps.create_lmove_message(pose_msg)
    if (connection_socket.send(command_string) is not len(command_string)):
        rospy.logerr("Proper command was not sent to the robot.")
    time.sleep(const.TIMEOUT_MEDIUM)
    message = connection_socket.recvfrom(const.RECEIVE_MSG_SIZE)
    feedback_command, feedback_error = ps.move_feedback_msg_parser(message)
    if ((feedback_command + b'\n') == command_string):
        if (feedback_error == const.MOTOR_OFF_ERROR):
            ...
            # TODO(ntonci): Handle any other issues that might occur.
            return True
    else:
...
def receive_after_motion_complete(connection_socket):
    ...
    message = connection_socket.recvfrom(const.RECEIVE_MSG_SIZE)
    if (message == const.MOTION_COMPLETE_MESSAGE):
        return True
    else:
        rospy.logerr('Received unknown error message:')
        rospy.loginf(message)
        return False
...
def compare_two_pose_msgs(current_pose, target_pose):
    ...
    """
    current_translation, current_euler = ps.convert_pose_msg_to_translation_euler(
        current_pose)
    target_translation, target_euler = ps.convert_pose_msg_to_translation_euler(
        target_pose)
    diff_t = 0.0
    diff_r = 0.0
    ...
    for i, j in zip(current_euler, target_euler):
        diff_r += pow(i - j, 2.0)
    if ((pow(diff_t, 0.5) < 0.0001) and (pow(diff_r, 0.5) < 0.05)):
        return True
    else:
        return False
